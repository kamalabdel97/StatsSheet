...
#Main Effect A (Store), Main Effect B (Gender), Interaction
...
StoreBFemale <- rnorm(50, 7000, 10) # Creates 365 samples of occurences, with a mean of 1000.12 and standard deviation of 332

Revenue <- c(StoreAMale, StoreAFemale, StoreBAMale, StoreBFemale)

Sales <- data.frame(Store, Gender, Revenue)

MainA_MainB_Intplot <- ggplot(Sales, aes(Store, Revenue, label = Revenue, fill = Gender)) + 
...
write.csv(Sales,
	file = "MainA_MainB_IntData.csv")

ggsave("MainA_NoMainB_IntPlot.jpeg",
	plot = MainA_MainB_Intplot)
