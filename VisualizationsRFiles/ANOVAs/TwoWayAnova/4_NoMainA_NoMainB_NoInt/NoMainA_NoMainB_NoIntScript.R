...
#No Main Effect A (Store), No Main Effect B (Gender), No Interaction
...
NoMainA_NoMainB_NoIntplot <- ggplot(Sales, aes(Store, Revenue, label = Revenue, fill = Gender)) + 
...
write.csv(Sales,
	file = "NoMainA_NoMainB_NoIntData.csv")

ggsave("NoMainA_NoMainB_NoIntPlot.jpeg",
	plot = NoMainA_NoMainB_NoIntplot)
