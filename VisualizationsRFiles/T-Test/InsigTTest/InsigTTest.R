...
StoreB <- rnorm(365, 4855.12, 332) # Creates 365 samples of occurences, with a mean of 1000.12 and standard deviation of 332
...
IngsigTTestplot <- ggplot(Sales, aes(Store, Revenue, label = Revenue, fill = Store)) + 
...
ggsave("InsigTTestPlot.jpeg", #Pathway to save plot to 
	IngsigTTestplot #Indicates to save the TTestplot 
)

write.csv(Sales, #Indicates to write the Sales dataframe
	"IngsigTTestData.csv" #Indicates the filepath to save the data to
)
